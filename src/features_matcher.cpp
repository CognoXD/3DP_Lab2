...
/**
 * Helper function to load features from the 'outputs' folder.
 * Reads perfectly formatted YAML files generated by OpenCV FileStorage in Python.
 */
bool loadExternalFeatures(const std::string& image_path, std::vector<cv::KeyPoint>& keypoints, cv::Mat& descriptors) {
    std::string feat_path = image_path;
    
    // 1. Replace "datasets" directory with "outputs" directory
    size_t pos = feat_path.find("datasets");
    if (pos != std::string::npos) {
        feat_path.replace(pos, 8, "outputs");
    }
    
    // 2. Change file extension from image format (.png/.jpg) to .yaml
    feat_path = feat_path.substr(0, feat_path.find_last_of('.')) + ".yaml";
    
    // Use OpenCV FileStorage to read the data
    cv::FileStorage fs(feat_path, cv::FileStorage::READ);
    if (!fs.isOpened()) {
        std::cerr << "Error: Could not open feature file: " << feat_path << std::endl;
        return false;
    }

    // Extract matrices from the YAML file
    cv::Mat kpts_mat;
    fs["keypoints"] >> kpts_mat;
    fs["descriptors"] >> descriptors;
    fs.release();

    // Reconstruct the cv::KeyPoint vector
    keypoints.clear();
    keypoints.reserve(kpts_mat.rows);
    for (int i = 0; i < kpts_mat.rows; ++i) {
        float x = kpts_mat.at<float>(i, 0);
        float y = kpts_mat.at<float>(i, 1);
        float score = kpts_mat.at<float>(i, 2);
...
void FeatureMatcher::extractFeatures()
{
  ...
  for( int i = 0; i < images_names_.size(); i++  )
  {
    ...
    if (use_modern_features_)
    {
      ...
      // OPTION B: Data Loading
      if (!loadExternalFeatures(images_names_[i], features_[i], descriptors_[i])) 
      {
          std::cerr << "CRITICAL ERROR: Feature file not found for " << images_names_[i] << std::endl;
          std::cerr << "Please run the Python script first!" << std::endl;
          exit(-1);
      }

      // Extract colors for each keypoint (required for MeshLab visualization)
      ...
      for( auto &f : features_[i])
      {
        ...
        // Store the BGR color from the original image at the keypoint location
  ...
  for( int i = 0; i < images_names_.size() - 1; i++ )
  {
    for( int j = i + 1; j < images_names_.size(); j++ )
    {
      ...
      if( use_modern_features_ )
      {
        ...
        // SIFT descriptors are floating-point vectors, so we use Euclidean distance (NORM_L2).
        // Setting crossCheck=true ensures higher quality by requiring mutual nearest neighbors.
      ...
      // We need to convert the DMatch objects into points to use OpenCV geometric functions
      std::vector<cv::Point2f> src_pts;
      std::vector<cv::Point2f> dst_pts;

      for (const auto& match : matches) {
          src_pts.push_back(features_[i][match.queryIdx].pt);
          dst_pts.push_back(features_[j][match.trainIdx].pt);
      }

      // If we don't have enough matches to even compute a matrix, skip this pair
      if (src_pts.size() < 8) continue;

      // 1. Geometric Verification using Essential Matrix
      cv::Mat inlier_mask_E;
      cv::Mat E = cv::findEssentialMat(src_pts, dst_pts, new_intrinsics_matrix_, cv::RANSAC, 0.999, 1.0, inlier_mask_E);

      // 2. Geometric Verification using Homography Matrix
      cv::Mat inlier_mask_H;
      cv::Mat H = cv::findHomography(src_pts, dst_pts, cv::RANSAC, 1.0, inlier_mask_H);

      // Combine inliers: a match is good if it is an inlier for E OR H
      // (Since SfM needs general motion (E) but must also survive planar scenes/pure rotation (H))
      for (size_t k = 0; k < matches.size(); k++) {
          bool is_inlier_E = !inlier_mask_E.empty() && inlier_mask_E.at<uchar>(k);
          bool is_inlier_H = !inlier_mask_H.empty() && inlier_mask_H.at<uchar>(k);
      ...
      // Do not set matches if the amount of verified matches is small
